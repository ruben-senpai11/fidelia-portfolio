import Image from "next/image";

import dress1 from "../assets/images/Dessin 1.png"
import dress2 from "../assets/images/Dessin 2.png"
// import dress3 from "../assets/images/Dessin 3.heic"
import dress4 from "../assets/images/Dessin 4.png"
import dress5 from "../assets/images/Dessin 5.png"
import dress6 from "../assets/images/Dessin 6.png"
import dress7 from "../assets/images/Dessin 7.png"
import dress8 from "../assets/images/Dessin 8.png"
import dress9 from "../assets/images/Dessin 9.png"
import dress10 from "../assets/images/Dessin 10.png"
import dress11 from "../assets/images/Dessin 11.png"
import dress12 from "../assets/images/Dessin 12.png"
import dress13 from "../assets/images/Dessin 13.png"
import dress14 from "../assets/images/Dessin 14.png"
import dress15 from "../assets/images/Dessin 15.png"
import dress16 from "../assets/images/Dessin 16.png"
import dress17 from "../assets/images/Dessin 17.png"
import dress18 from "../assets/images/Dessin 18.png"
import dress19 from "../assets/images/Dessin 19.png"
import dress20 from "../assets/images/blue_dressing.png"
import dress21 from "../assets/images/Dessin 21.png"
import Crea from "./components/creationPresentation";


export default function Page() {

  return (
    <>
      <main className="flex flex-col items-center justify-between py-40 ">
        <section id="creations" className="w-full overflow-hidden ">
          <div className="crea-grid flex justify-between flex-shrink-0 gap-24">
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress1} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress2} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress4} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress4} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress5} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress6} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress7} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress8} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress9} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress10} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress11} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress12} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress13} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress14} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress15} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress16} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress17} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress18} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress19} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress20} 
              imageAlt="Robe sicintillante étoilée" 
            />
            <Crea 
              title="Robe étoilée" 
              description="Robe de soirée somptueuse aux éclats de la lune et au rire du soleil avec une touche de parfum des étoiles brillantes scintillantes " 
              imageSrc={dress21} 
              imageAlt="Robe sicintillante étoilée" 
            />
          </div>
        </section>
      </main>
    </>
  )
}